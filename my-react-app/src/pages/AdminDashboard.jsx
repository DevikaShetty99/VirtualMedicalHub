...
//   useEffect(() => {
//     const stored = localStorage.getItem("sentComplaints");
//     if (stored) setSentComplaints(new Set(JSON.parse(stored)));
//   }, []);
...
//             Manage doctors, patients, reports and complaints from here.
//           </Typography>
//         </Box>
//       </Paper>
...
//       {/* The rest of your component remains unchanged (Patients, Reports, Complaints)... */}
//       <Paper elevation={3} sx={{ p: 4, mb: 4 }}>
//         {/* <Box sx={{ mb: 2, display: "flex", justifyContent: "center" }}>
//           <Button variant="contained" color="success" onClick={fetchPatients}>
//             {showPatients ? "Refresh Patients" : "View Patients"}
//           </Button>
//           {showPatients && (
...
//             {showReports ? "Refresh Report" : "Generate Report"}
//           </Button>
//           {showReports && (
//             <>
...
//         {showComplaints ? "Refresh Complaints" : "View Complaints"}
//         </Button>
//         {showComplaints && (
...
export default function AdminDashboard() {
  ...
  // const handleFeedbackResponseChange = (id, value) => {
  //   setFeedbackResponse((prev) => ({ ...prev, [id]: value }));
  // };

  // const respondToFeedback = (item) => {
  //   const responseText = feedbackResponse[item.id];
  //   if (!responseText) return;

  //   // Mark as responded
  //   setRespondedFeedback((prev) => new Set(prev).add(item.id));

  //   // Optionally, store the response in localStorage
  //   const updatedFeedback = feedback.map((fb) =>
  //     fb.id === item.id ? { ...fb, response: responseText } : fb
  //   );
